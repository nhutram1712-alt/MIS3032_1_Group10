# Prototype Documentation & Usability Test Report
...
1. Prototype đã chạy được (mở `prototype.html` bằng trình duyệt) cho cả 4 flow.
2. Đã tự kiểm tra đủ state theo checklist ở `huong-dan-buoc-3-den-7.md` Bước 4.
3. Chuẩn bị công cụ ghi lại: quay màn hình, hoặc ngồi cạnh ghi chú tay theo mẫu bên dưới.
4. Chọn 3 người test, cố gắng match với 1 trong các persona: Requester (dễ nhất, ai cũng đóng được vai sinh viên/GV), Facility Manager, Technician.
...
| # | Người test | Vai trò đóng | Ngày test | Ghi chú |
|---|---|---|---|---|
...
| Flow | Thanh Thảo | Thu Thảo | Tuấn / Trầm | Tỷ lệ hoàn thành |
|---|---|---|---|---|
...
1. **[Nghiêm trọng nhất] OQ-01** — Technician hiểu nhầm trạng thái Work Order sau khi từ chối (chi tiết bên dưới).
2. **Q-13** — Người dùng (vai Facility Manager) mong muốn thấy thêm thông tin định lượng (%) bên cạnh nhãn Risk, không chỉ Low/Medium/High.
3. Không phát hiện vấn đề nghiêm trọng nào ở Flow 1 và Flow 2 — cả 2 flow được hoàn thành mượt, không cần thay đổi lớn.
...
**Người test (Trầm) kỳ vọng gì sau khi từ chối WO:**
> Sau khi bấm "Xác nhận từ chối" và thấy màn hình xác nhận ghi "Work Order vẫn giữ trạng thái Assigned", người test nói: *"Ủa sao vẫn Assigned? Tôi tưởng từ chối xong là xong việc của tôi rồi, không liên quan tới tôi nữa chứ?"* — người test quay lại danh sách Work Order và thấy công việc đó vẫn nằm trong danh sách được giao cho mình, tỏ ra bối rối, hỏi thêm: *"Vậy giờ tôi có phải làm gì nữa không, hay Facility Manager sẽ tự thấy?"*
...
| ID | Nội dung | Kết quả từ test | Cập nhật vào open-questions.md? |
|---|---|---|---|
| **Q-01** | Requester xem Asset theo phòng hay toàn bộ? | Không phát sinh vấn đề trong test (Thảo chỉ thấy đúng 1 tài sản mẫu thuộc phòng của mình, không thắc mắc gì) — chưa đủ dữ liệu để kết luận chắc chắn | **O** *(giữ Open, cần test thêm với nhiều Asset hơn)* |
...
| **Q-05** | FM có cần approve riêng trước khi tạo WO? | Thu Thảo (đóng vai FM) tự nhiên đọc kỹ nội dung trước khi tạo WO mà không cần có nút "Approve" tách riêng — hành vi "đọc kỹ rồi mới bấm Tạo" đã đóng vai trò tương đương approve | **X** *(đề xuất Resolved: không cần bước approve riêng, gộp chung với hành động Tạo Work Order như hiện tại)* |
| **Q-08** | FM có cần xác nhận trước khi đóng Request? | Chưa được test trực tiếp trong lần mô phỏng này (thiếu thời gian) | **O** *(giữ Open, cần test riêng bước "Xác nhận kết quả & Đóng yêu cầu")* |
| **Q-13** | Maintenance Risk hiển thị Low/Medium/High hay kèm %? | Thu Thảo chủ động hỏi thêm về độ tin cậy/con số cụ thể dù không có trên UI | **O** *(giữ Open nhưng nghiêng về hướng cần bổ sung thêm thông tin định lượng — cần test với nhiều Facility Manager hơn để chắc chắn)* |

---

### Hạn chế của đợt test mô phỏng này (cần nêu rõ khi báo cáo)
1. **Cỡ mẫu rất nhỏ:** chỉ 1 người/vai trò, mỗi flow chỉ được test 1 lần — không đủ để kết luận thống kê, chỉ đủ để phát hiện tín hiệu ban đầu (early signal).
2. **Task 3b (quan trọng nhất — OQ-01)** chỉ có 1 lượt quan sát, nên đề xuất Phương án 2 cần được xác nhận thêm trước khi áp dụng chính thức.
3. Đây là dữ liệu mô phỏng, không phải người dùng thật của Trường Đại học Kinh tế – Đại học Đà Nẵng — khi triển khai thật, cần lặp lại quy trình này với đúng đối tượng (giảng viên/sinh viên/nhân viên kỹ thuật thật).
...
### DEC-03 — Xác nhận: không cần bước "Approve" tách riêng trước khi tạo Work Order
* **Ngày:** 03/09/2026
...
| **Q-02** | Maintenance Request có bắt buộc đính kèm hình ảnh/video không? | Open | Resolved (đề xuất) | Usability test xác nhận label "(không bắt buộc)" đủ rõ, giữ nguyên theo REQ-04. Xem DEC-02. |
| **Q-05** | Facility Manager có cần approve Maintenance Request trước khi tạo Work Order không? | Open | Resolved (đề xuất) | Usability test cho thấy hành vi đọc kỹ trước khi tạo WO đã thay thế vai trò approve riêng. Xem DEC-03. |

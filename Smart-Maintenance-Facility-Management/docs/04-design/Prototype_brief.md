...
1. **UC-03** — Danh sách Asset theo phòng/khu vực (REQ-02, REQ-03)
2. **UC-05** — Form tạo Maintenance Request (mô tả vấn đề + Asset/khu vực + ảnh không bắt buộc)
3. **UC-05** — Xác nhận đã gửi (trạng thái khởi tạo = `Submitted`)
4. **UC-06** — Danh sách Request của tôi (hiển thị đủ 6 trạng thái)
5. **UC-06** — Chi tiết 1 Request

## States bắt buộc

- `loading`
- `empty` — chưa có Asset được gán / chưa có Request nào — UC-06 A1
...
- **BR-05:** Request phải xác định **Asset hoặc khu vực** bị ảnh hưởng.
  - Nếu chỉ chọn khu vực (chưa chọn Asset cụ thể), vẫn được phép tạo Request (AC-US-03-01-02).
  - Tuy nhiên, phải xác định Asset trước khi tạo Work Order. Ràng buộc này thuộc Flow 2, UC-08 A2.
- **BR-14 / AC-US-03-02-02:** Đúng 6 trạng thái chính thức:
  - `Submitted → Pending → In Progress → Resolved → Closed`
  - hoặc nhánh `Submitted → Rejected`

## Sample Data

- **Asset:** `AC-A203-01` — Điều hòa, Phòng A203, Air Conditioner
- **Request mẫu 1:** `REQ-1042` — "Điều hòa không lạnh, có tiếng ồn lớn" — `Pending`
- **Request mẫu 2:** `REQ-1040` — "Wi-Fi rớt liên tục khi đông người" — `Resolved`
- **Request mẫu 3:** `REQ-1039` — "Hình ảnh máy chiếu mờ, chập chờn" — `In Progress`
- **Request mẫu 4:** `REQ-1021` — "Rò rỉ nước ở dàn lạnh" — `Closed`
...
1. **UC-07** — Danh sách Maintenance Request chờ xử lý
2. **UC-07** — Chi tiết Request + nút **"Tạo Work Order"**
3. **UC-08** — Form tạo Work Order (chọn Technician)
4. **UC-08** — Xác nhận đã tạo & phân công (trạng thái WO = `Assigned`)
5. **UC-10** — Danh sách Work Order + trạng thái (`Assigned` / `In Progress` / `Completed` / `Cancelled` theo BR-17)
6. **UC-07 (bước 9-11) / US-03-04** — Màn hình xác nhận kết quả & đóng Request (chỉ xuất hiện khi WO liên quan đã `Completed` → Request tự chuyển `Resolved` → FM xác nhận → `Closed`)
...
- Prototype gộp **"Tạo Work Order"** và **"Phân công Technician"** thành 1 form duy nhất để thao tác nhanh.
- Thực tế UC-08 mô tả 2 bước tách biệt: bước 5 tạo WO, bước 6-7 mới chọn & phân công Technician.
- Cần test xem FM có cảm thấy thiếu bước **"xem lại trước khi phân công"** không.
...
1. **UC-09** — Danh sách Work Order được phân công
2. **UC-09** — Chi tiết Work Order (Asset info + IoT Alert + AI Prediction nếu có — theo UC-15 bước 5: Technician chỉ xem thông tin liên quan Asset trong WO được giao)
3. **UC-09 (A1)** — Từ chối kèm lý do bắt buộc
4. **UC-09** — Form ghi nhận kết quả xử lý
5. **UC-09/UC-10** — Xác nhận hoàn thành (WO chuyển `Completed`, lưu vào Maintenance History)

## States bắt buộc

- `empty`
- `loading`
- 2 biến thể ở màn hình chi tiết:
  - **Có** IoT Alert/AI Prediction
  - **Không có** IoT Alert/AI Prediction — UC-15 A1: "Không có Prediction → hệ thống thông báo chưa có AI Prediction"
...
- Sau khi Technician **Hoàn thành (`Completed`)**, Request liên quan tự chuyển `Resolved`.
- Technician chưa thấy rõ đây có phải bước cuối hay còn chờ FM xác nhận (Q-08 vẫn Open).
- Cần quan sát xem Technician có hiểu rõ luồng xử lý sau khi hoàn thành WO hay không.
...
1. **UC-15** — Dashboard tổng quan: danh sách Asset kèm Maintenance Risk (`Low` / `Medium` / `High`)
2. **UC-12** — Chi tiết IoT Data của 1 Asset
3. **UC-14/UC-15** — Chi tiết AI Prediction (risk level + khuyến nghị)
4. **UC-08** — Hành động: **Tạo Work Order chủ động** từ Prediction (nối sang Flow 2)

## States bắt buộc

- `loading`
- `empty` — chưa có Asset nào có Alert/Risk
...
- **BR-10 / AC-US-06-01-04:** AI chỉ cung cấp Prediction và **không tự động quyết định hoặc thực hiện bảo trì**.
- Prototype phải luôn có bước FM tự bấm **"Tạo Work Order chủ động"**; AI không tự tạo.
...
- Risk hiển thị dạng nhãn `Low / Medium / High` (REQ-12), **không hiển thị % cụ thể** — Q-13 vẫn Open, cần test xem FM có muốn thấy con số % không.
